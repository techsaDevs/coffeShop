...
const BasketPage = () => {
  ...
  const sortedProducts = [...productsInBasket].sort((a, b) => {
    switch (sortBy) {
      ...
      case "star-asc":
        return getAverageStars(a.starCount) - getAverageStars(b.starCount);
      case "star-desc":
        return getAverageStars(b.starCount) - getAverageStars(a.starCount);
      case "qty-asc":
        return a.qty - b.qty;
      case "qty-desc":
        return b.qty - a.qty;
    ...
    return (
      <Container className="min-h-[58.5vh] py-20 flexCenter">
        <div className="md:mt-14 lg:mt-17 flex flex-col items-center gap-6">
          ...
          <Link href="/login" className="basketBtn text-lg px-[27px] py-[13px]">
            ورود | ثبت‌نام
          </Link>
        </div>
      </Container>
            ...
            <div className="flex flex-col justify-between flex-1">
              ...
              <div className="flex justify-between items-center mt-3">
                  ...
                  <div className="flex flex-col">
                    {off > 0 && (
                      <div className="">
                        <span className="text-foregreen font-dana-medium tracking-tighter text-xs">
                          {((price * off) / 100).toLocaleString()} تومان تخفیف
                        </span>
                        <span className="inline-block border-foregreen border-1 rounded-full size-7 leading-7 mr-2 text-foregreen text-xs text-center">
                          {off}%
                        </span>
                      </div>
                    )}
                    <div className="space-x-1">
                      <span className="font-dana-dbold text-lg">
                        {(price - ((price * off) / 100)).toLocaleString()}
                      </span>
                ...
                <div className="flex items-center gap-2 bg-background rounded-full px-2 py-1">
                  ...
                  <span className="px-3 py-1 text-sm font-medium text-orange-500">
                    {qty}
                  </span>
                  <button
                    className="p-1 rounded-full hover:bg-foregray group transition-colors"
